...
class CategoryModel extends DateBaseModel  
{
	public $categories;
	...
	{ 
		$sql = sprintf("SELECT title FROM category WHERE id='%d'", mysqli_real_escape_string(mysqli_connect(HOST, USER, PASSWORD, 'Sofastore'),$id));
		$result = mysqli_query(mysqli_connect(HOST, USER, PASSWORD, 'Sofastore'),$sql)  or die(mysqli_error(mysqli_connect(HOST, USER, PASSWORD, 'Sofastore')));
